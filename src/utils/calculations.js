...
/**
 * Higher level calculation for the entire record including Tax, Discount, and Advance
 */
export const calculateGrandTotal = (trips, options = {}) => {
  const taxRate = Number(options.taxRate) || 0;
  const discount = Number(options.discount) || 0;
  const advancePaid = Number(options.advancePaid) || 0;
  ...
  const taxableAmount = Math.max(0, subtotal + extraChargesTotal - discount);
